...
!!! note
    ...
    `caused_failure` digs to the deepest failing leaf, even inside nested workflows. `threw_failure` is the immediate upstream (`origin` or the result itself). When the failing task *is* the leaf you are looking at, both helpers return `self`. More detail: [Result — Chain Analysis](outcomes/result.md#chain-analysis).
...
Middleware wraps the real task lifecycle, so the friendliest test is a tiny task that exercises your middleware end-to-end. See [Middlewares](middlewares.md) for the big picture.
