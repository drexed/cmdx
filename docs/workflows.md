...
Because a workflow is still a task, you keep all the goodies: [inputs](inputs/definitions.md), [callbacks](callbacks.md), [middlewares](middlewares.md), [settings](configuration.md#settings), [outputs](outputs.md), and [retries](retries.md). Use those hooks to validate at the workflow level, warm up shared state, or react when things go sideways.
...
2. When the whole pipeline gives up, **Pipeline** walks successful tasks in **reverse** run order and calls `#rollback` on any that define it. That is the saga-style cleanup.
