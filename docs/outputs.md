...
Outputs stay intentionally small. If you need coercion, fancy validation, or nested resolution, use [Inputs](inputs/definitions.md) (or plain Ruby after `work`).
...
Curious about every default shape in detail? [Inputs - Defaults](inputs/defaults.md) spells it out — **same rules** as here.
...
2. Otherwise read `context[name]`.
3. If it is `nil`, try `:default`.
4. Still nothing? That is a missing output (`cmdx.outputs.missing`).
5. Otherwise write the resolved value back to `context[name]`.
