...
1. **Confirm Ruby.** You need Ruby 3.3+ (MRI, JRuby, or TruffleRuby). Details: [Getting Started](https://drexed.github.io/cmdx/getting_started/#requirements).
1. **Keep an escape hatch.** Pin v1 in your `Gemfile` first, e.g. `gem "cmdx", "~> 1.21"`, so you can roll back if the upgrade needs more time.
1. **Save a green baseline.** On v1, run `bundle exec rspec` once and keep the output. That is your “before” photo.
1. **Peek at the changelog.** The `[2.0.0]` section in [`CHANGELOG.md`](https://github.com/drexed/cmdx/blob/main/CHANGELOG.md) lists breaks grouped by topic.
1. **Skim this page in order.** Each section stands alone, but reading top to bottom matches how most teams migrate.
...
1. **Fix configuration first.** Remove keys that no longer exist ([Configuration](#configuration)). `rails generate cmdx:install` prints a fresh v2 initializer you can copy ideas from.
1. **Fix tasks in layers.** Inputs, then outputs, then callbacks, then middleware, then anything that reads `Result`. The [Automated Migration Prompt](#automated-migration-prompt) automates a big chunk of that if you want help.
1. **Hunt old `Result` habits.** Look for `result.executing?`, writes to `result.metadata`, `result.good?` / `bad?`, and breakpoint-style config. v2’s `Result` is calmer and stricter.
1. **Recreate observability.** Correlation IDs, timing, timeouts — use [Telemetry](#telemetry) and/or your own middleware instead of the removed built-ins.
1. **Green tests, then cleanup.** Once the suite passes, delete v1-only shims (`dry_run:`, `SKIP_CMDX_FREEZING`, manual rollback hacks you no longer need).
1. **Run the straggler grep.** [Validating the Migration](#validating-the-migration) has a copy-paste command to catch leftovers.
...
See [Configuration](https://drexed.github.io/cmdx/configuration/index.md) for every option explained in one place.
...
More detail: [Outcomes - Result](https://drexed.github.io/cmdx/outcomes/result/index.md).
...
- Fine tuning: pick an `:executor` (`:threads` by default, `:fibers`, or your own callable), a `:merger` (`:last_write_wins`, `:deep_merge`, `:no_merge`, or custom), and whether to `continue_on_failure`. [Workflows - Parallel Execution](https://drexed.github.io/cmdx/workflows/#parallel-execution) has examples.
...
Full guide: [Workflows](https://drexed.github.io/cmdx/workflows/index.md).
...
Heads-up: `result.chain` type changed
...
More patterns: [Deprecation](https://drexed.github.io/cmdx/deprecation/index.md).
...
Each event is a `Telemetry::Event` (`cid`, `root`, `type`, `task`, `tid`, `name`, `payload`, `timestamp`). You can also subscribe on a single task class with `MyTask.telemetry.subscribe(...)`. [Configuration - Telemetry](https://drexed.github.io/cmdx/configuration/#telemetry) lists everything you can tweak.
...
More: [Internationalization](https://drexed.github.io/cmdx/internationalization/index.md).
...
1. Pin CMDx again, e.g. `gem "cmdx", "~> 1.21"`.
1. Bring back any small helpers you deleted (manual rollback wiring, breakpoint YAML, `dry_run` branches).
...
- **`Chain#to_timeline`** — rows of `(task, start, end, status)` for dashboards.
