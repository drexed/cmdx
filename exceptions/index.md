...
Raised when a task with `settings(deprecate: :raise)` is executed. See [Deprecation](https://drexed.github.io/cmdx/deprecation/index.md).
...
Base class for execution interruptions raised by `execute!`. All faults carry a `result` with full execution context. See [Faults](https://drexed.github.io/cmdx/interruptions/faults/index.md) for advanced matching.
...
Raised by the [Timeout middleware](https://drexed.github.io/cmdx/middlewares/#timeout) when a task exceeds its time limit.
