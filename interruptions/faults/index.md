...
A fault wraps the **originating** failed `Result` — the leaf at the bottom of any `throw!` chain — and delegates `task`, `context`, and `chain` from that result. For the bigger picture on errors, see [Exceptions](https://drexed.github.io/cmdx/interruptions/exceptions/index.md).
...
`fault.result` is the originating failure; `fault.chain` is the whole story. Walk propagation with `origin`, `caused_failure`, and `threw_failure`. Full tour in [Result - Chain Analysis](https://drexed.github.io/cmdx/outcomes/result/#chain-analysis).
