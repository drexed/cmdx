...
RSpec.describe CMDx::Middlewares::Timeout, type: :unit do
  ...
  describe ".call" do
    context "when seconds option is a Numeric" do
      it "uses the numeric value as timeout limit coerced to float" do
        expect(Timeout).to receive(:timeout).with(5.0, CMDx::TimeoutError, "execution exceeded 5.0 seconds").and_yield.and_return(block_result)
      ...
      it "yields directly when timeout is zero" do
        expect(Timeout).not_to receive(:timeout)

        result = timeout_middleware.call(task, seconds: 0, &test_block)

        expect(result).to eq(block_result)
      end

      it "yields directly when timeout is negative" do
        expect(Timeout).not_to receive(:timeout)

        result = timeout_middleware.call(task, seconds: -1, &test_block)
    ...
    context "when seconds option is a Symbol" do
      ...
      it "calls the method on the task and uses the result as float" do
        allow(task).to receive(:send).with(method_name).and_return(10)

        expect(Timeout).to receive(:timeout).with(10.0, CMDx::TimeoutError, "execution exceeded 10.0 seconds").and_yield.and_return(block_result)

        timeout_middleware.call(task, seconds: method_name, &test_block)
      end

      it "raises ArgumentError when method returns non-numeric value" do
    ...
    context "when seconds option is a Proc" do
      ...
      it "evaluates the proc in task context and uses the result as float" do
        expect(task).to receive(:instance_eval).and_yield.and_return(15)

        expect(Timeout).to receive(:timeout).with(15.0, CMDx::TimeoutError, "execution exceeded 15.0 seconds").and_yield.and_return(block_result)
    ...
    context "when seconds option responds to call" do
      ...
      it "calls the callable with the task and uses the result as float" do
        allow(callable).to receive(:call).with(task).and_return(20)

        expect(Timeout).to receive(:timeout).with(20.0, CMDx::TimeoutError, "execution exceeded 20.0 seconds").and_yield.and_return(block_result)

        timeout_middleware.call(task, seconds: callable, &test_block)
      end

      it "yields directly when callable returns zero" do
        allow(callable).to receive(:call).with(task).and_return(0)

        expect(Timeout).not_to receive(:timeout)

        result = timeout_middleware.call(task, seconds: callable, &test_block)
    ...
    context "when seconds option is nil" do
      it "uses the default timeout limit as float" do
        expect(Timeout).to receive(:timeout).with(3.0, CMDx::TimeoutError, "execution exceeded 3.0 seconds").and_yield.and_return(block_result)
