...
RSpec.describe CMDx::Chain do
  ...
  describe "delegated methods to results" do
    before do
    ...
    describe "#first" do
      it "delegates to results array" do
        expect(chain.first).to eq(mock_result)
      end
    end

    describe "#last" do
      it "delegates to results array" do
        expect(chain.last).to eq(mock_result2)
      end
    end
  ...
  describe "delegated methods to first result" do
    let(:mock_state) { "complete" }
    let(:mock_status) { "success" }
    let(:mock_outcome) { "success" }

    before do
      allow(mock_result).to receive_messages(
        state: mock_state,
        status: mock_status,
    ...
    describe "#state" do
      it "delegates to first result" do
        expect(chain.state).to eq(mock_state)
      end
    end

    describe "#status" do
      it "delegates to first result" do
        expect(chain.status).to eq(mock_status)
      end
    end

    describe "#outcome" do
      it "delegates to first result" do
        expect(chain.outcome).to eq(mock_outcome)
      end
    end

    context "when results array is empty" do
      before { chain.results.clear }

      it "raises NoMethodError for delegated methods when first is nil" do
