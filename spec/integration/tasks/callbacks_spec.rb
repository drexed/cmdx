...
RSpec.describe "Task callbacks", type: :feature do
  ...
  describe "failure cause on task" do
    it "exposes the rescued exception on on_failed callbacks" do
      task = create_erroring_task(reason: "broken") do
        on_failed { context.cause_class = cause.class.name }
      end

      expect(task.execute.context[:cause_class]).to eq("CMDx::TestError")
    end

    it "exposes the rescued exception in around_execution after the continuation" do
      task = create_task_class(name: "AroundCause") do
        around_execution(proc { |t, cont|
          cont.call
          t.context.cause_message = t.cause&.message
        })
        define_method(:work) { raise CMDx::TestError, "around" }
      end

      expect(task.execute.context[:cause_message]).to eq("around")
    end

    it "exposes the rescued exception on rollback" do
      task = create_erroring_task(reason: "rollback me") do
        define_method(:rollback) { context.cause_message = cause&.message }
      end

      expect(task.execute.context[:cause_message]).to eq("rollback me")
    end

    it "leaves cause nil for intentional fail! halts" do
      task = create_failing_task(reason: "nope") do
        on_failed { context.cause_present = !cause.nil? }
      end

      expect(task.execute.context[:cause_present]).to be(false)
    end

    it "exposes an explicit fail! cause on on_failed callbacks" do
      error = CMDx::TestError.new("declined")
      task = create_task_class(name: "FailWithCause") do
        define_method(:work) { fail!("Card declined", cause: error) }
        on_failed { context.cause_message = cause&.message }
      end

      result = task.execute

      expect(result).to be_failed
      expect(result.cause).to eq(error)
      expect(result.context[:cause_message]).to eq("declined")
    end

    it "exposes throw! propagated cause on on_failed callbacks" do
      inner = create_erroring_task(reason: "nested")
      outer = create_task_class(name: "ThrowCause") do
        define_method(:work) { throw!(inner.execute(context)) }
        on_failed { context.cause_message = cause&.message }
      end

      expect(outer.execute.context[:cause_message]).to eq("nested")
    end

    it "leaves cause nil on success" do
      captured = nil
      task = create_successful_task do
        on_success { captured = cause }
      end
    ...
    it "supports conditional callbacks gated on the exception class" do
      task = create_erroring_task(reason: "retryable") do
        on_failed(:note_retryable, if: :retryable_error?)
        on_failed(:note_other, unless: :retryable_error?)
        define_method(:retryable_error?) { cause.is_a?(CMDx::TestError) }
        define_method(:note_retryable) { (context.log ||= []) << :retryable }
        define_method(:note_other) { (context.log ||= []) << :other }
      end

      expect(task.execute.context[:log]).to eq([:retryable])
    end

    it "mirrors result.cause on the task after execution" do
      task = create_erroring_task(reason: "synced").new
      result = task.execute

      expect(task.cause).to eq(result.cause)
      expect(task.cause.message).to eq("synced")
    end
  end
end
